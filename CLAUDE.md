...
- **public/index.html（スマホ用ドライバー画面）** — 全画面実装済み・DB接続済み
  ...
  - 前回帰社ODOの自動引き継ぎ
  - **計画モード + フリーモード両対応**（タブ切り替え）
    - 計画タブ: 当日の planned レポート一覧 → 選択 → 車輌選択 → 出発
    - フリータブ: 従来の車輌・日付・コース選択
  - 計画モードのフロー: 次の届け先を重量登録済み優先で自動セット、「別の届け先を選ぶ」トグルで順番変更可
  - 計画モードにスキップ機能なし（全配達先を順不同で配達するフロー）
  - 計画モードの件数表示: 重量が登録されている stop_records 件数のみカウント
- **supabase/schema.sql** — スキーマ・RLS・インデックス・ビュー・Realtime設定
- **supabase/seed.sql** — テスト用シードデータ
- **supabase/migrate_add_plan_status.sql** — Phase 5 マイグレーション（Supabase ダッシュボードで実行が必要）
- **public/admin.html（管理者画面）** — Dashboard・配送計画・レポート・CSVダウンロード・マスタ管理 実装済み
  ...
  - サイドメニュー: Dashboard / 配送計画 / 日報編集 / レポート / CSVダウンロード / マスタ管理
  - レポート: 直近1か月の総重量・配送回数・走行距離グラフ（支店フィルター付き、Chart.js）
  - CSVダウンロード: 得意先別集計に販売管理得意先コード列追加
  - **配送計画**: 日付・コース選択（コース選択で自動読み込み）→ 配達先に重量入力 → 保存
    - 重量が入力された配達先のみ計画に含まれる（チェックボックスなし）
    - 重量が1件も入力されていない場合は保存不可
    - 計画一覧: 重量登録済み件数・総重量・ステータス・車輌を表示
...
- **配達順序（計画モード）**: 順番変更可、スキップなし、全件配達が前提
- **計画モードの「件数」定義**: `stop_records` のうち `weight_kg IS NOT NULL AND weight_kg > 0` の件数
- **計画モードの包含ルール**: 重量あり = 配達する、重量なし = 計画に含まない
